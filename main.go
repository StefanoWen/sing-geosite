...
func download(release *github.RepositoryRelease) ([]byte, error) {
	geositeAsset := common.Find(release.Assets, func(it *github.ReleaseAsset) bool {
		return (*it.Name == "dlc.dat") || (*it.Value == "geosite.dat");
	})
	geositeChecksumAsset := common.Find(release.Assets, func(it *github.ReleaseAsset) bool {
		return (*it.Name == "dlc.dat.sha256sum") || (*it.Value == "geosite.dat.sha256sum");
	})
	if geositeAsset == nil {
...
func main() {
	err := release("Loyalsoldier/v2ray-rules-dat", "StefanoWen/sing-geosite", "geosite.db")
	if err != nil {
		logrus.Fatal(err)
	}
}
